...
def RunBoosting(dataset_loc):
    ...
    plt.savefig("out/Bagging_Algorithm.png")
    plt.clf()
...
def RunDefaultPrediction(dataset_loc):
    ...
    print(f"\ntree: training error = {train_tree}, testing error = {test_tree}")
    ...
    print(f"\nbagged trees: training error = {train_bag[-1]}, testing error = {test_bag[-1]}")
    print(f"\nrandom forest: training error = {train_rf[-1]}, testing error = {test_rf[-1]}")
    ...
    plt.savefig("out/Bonus_Question.png")
